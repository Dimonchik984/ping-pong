...
class GameServer:
    ...
    def reset_game_state(self):
        ...
        self.score = [0,0]
        self.ball = {"x": WIDTH // 2,"y": HEIGHT // 2, "vx": BALL_SPEED*random.choice([-1,1]), "vy": BALL_SPEED*random.choice([-1,1])}
    ...
    def handle_client(self, pid):
        ...
        try:
            ...
            with self.lock:
                self.connect[pid] = False
                self.game_over = True
                self.winner = 1 - pid
                print(f"Гравець {pid} відключився і переміг гравець {1-pid}")
    ...
    def ball_logic(self):
        ...
        while not self.game_over:
            with self.lock:
                ...
                if (self.ball['x'] <= 40 and self.paddles[0] <= self.paddles[0] + 100) or \
